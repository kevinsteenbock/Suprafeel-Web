...
export function AdminCampana() {
  ...
  const all = (materialsOf(c.id).length ? materialsOf(c.id) : materialsOf('otono')).filter((m) => !removed.includes(m.id))
  ...
  return (
    <Screen toolbar={<Toolbar back="/admin/plv" crumbs={[{ label: 'Materiales PLV', to: '/admin/plv' }, { label: name }]} right={<><SearchField placeholder="Buscar material" /><Link to={`/admin/plv/${c.id}/nuevo-material`}><Button variant="primary" icon={<Plus size={13} strokeWidth={2.4} />}>Añadir material</Button></Link></>} />}>
      ...
      {vista.mode === 'list' ? (
        <TableCard footer={`${all.length} materiales · ${all.filter((m) => m.kind !== 'digital').length} impresos y ${all.filter((m) => m.kind === 'digital').length} digitales · 1 sin publicar por falta de foto`} footerRight={<button onClick={() => notify(`${c.requests} solicitudes`)}>Ver {c.requests} solicitudes</button>}>
