...
export function MiComercial() {
  const { miLista, savedCases, solicitud, notify } = useStore()
  const plvCount = Object.values(solicitud).reduce((a, b) => a + b, 0)
  return (
    <Screen toolbar={<Toolbar title="Mi comercial" right={<Button variant="primary" icon={<CalendarDays size={13} />} onClick={() => notify('Le hemos propuesto a Álvaro adelantar la visita')}>Pedir una visita</Button>} />}>
      ...
      <div className="flex gap-6">
        ...
        <aside className="w-[316px] shrink-0 flex flex-col gap-4">
          <div className="rounded-xl bg-surface border border-line shadow-card overflow-hidden">
            <div className="h-9 px-4 bg-chrome border-b border-line flex items-center"><span className="label">Lo que verá antes de venir</span></div>
            {[
              { k: 'Mi lista', v: `${miLista.length} productos`, to: '/farmacia/mi-lista' },
              { k: 'Casos guardados', v: `${savedCases} del consejo`, to: '/farmacia/consejo' },
              { k: 'Material PLV pedido', v: `${plvCount} unidades`, to: '/farmacia/plv' },
              { k: 'Formación del equipo', v: '72 % · 1 certificado', to: '/farmacia/formacion' },
            ].map((r, i) => (
              <Link key={r.k} to={r.to} className={cn('h-[46px] px-4 flex items-center gap-3 hover:bg-canvas', i < 3 && 'border-b border-line-soft')}>
                <span className="flex-1 text-base text-ink">{r.k}</span><span className="text-sm text-muted">{r.v}</span><ChevronRight size={14} className="text-faint" />
              </Link>
            ))}
          </div>
          ...
          <Link to="/farmacia/ajustes/farmacia" className="text-base font-medium text-accent">Solicitar cambio de comercial →</Link>
        </aside>
      </div>
    </Screen>
  )
